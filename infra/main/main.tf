...
resource "azurerm_application_gateway" "appgw" {
  ...
  # SSL Certificate (solo si provees certificado)
  dynamic "ssl_certificate" {
    for_each = var.cert_data != "" ? [1] : []
    content {
      name     = "cert-multimodulo"
      data     = var.cert_data
  ...
  dynamic "http_listener" {
    ...
    content {
      ...
      protocol                       = "Https"
      ssl_certificate_name           = "cert-multimodulo"
    }
  }

  # HTTP Listener (siempre disponible)
  http_listener {
    name                           = "listener-http"
    frontend_ip_configuration_name = "appgw-frontend-ip"
  ...
  backend_http_settings {
    ...
    host_name             = "${var.app_service_name_web}.azurewebsites.net"
    cookie_based_affinity = "Disabled"
  }
  ...
  probe {
    name                 = "probe-backend"
    protocol             = "Https"
    host                 = "${var.app_service_name}.azurewebsites.net"
  ...
  probe {
    name                 = "probe-frontend"
    protocol             = "Https"
    host                 = "${var.app_service_name_web}.azurewebsites.net"
    path                 = "/"
    interval             = 30
    timeout              = 30
    unhealthy_threshold  = 3
  ...
  request_routing_rule {
    ...
    http_listener_name = var.cert_data != "" ? "listener-https" : "listener-http"
