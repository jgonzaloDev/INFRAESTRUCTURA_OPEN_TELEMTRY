# ============================================================
# VARIABLES PRINCIPALES
# ============================================================

variable "subscription_id" {
  description = "ID de la suscripción de Azure"
  type        = string
}

variable "tenant_id" {
  description = "ID del tenant de Azure"
  type        = string
}

variable "location" {
  description = "Ubicación de los recursos (por ejemplo: eastus2)"
  type        = string
}

variable "resource_group_name" {
  description = "Nombre del grupo de recursos"
  type        = string
}

# ============================================================
# VIRTUAL NETWORK
# ============================================================

variable "vnet_name" {
  description = "Nombre de la red virtual principal"
  type        = string
}

# ============================================================
# APP SERVICE PLANS Y WEB APPS
# ============================================================

variable "app_service_plan_name" {
  description = "Nombre del App Service Plan para backend (Spring Boot)"
  type        = string
}

variable "app_service_plan_name_web" {
  description = "Nombre del App Service Plan para frontend (React)"
  type        = string
}

variable "app_service_name" {
  description = "Nombre del App Service backend (Spring Boot)"
  type        = string
}

variable "app_service_name_web" {
  description = "Nombre del App Service frontend (React)"
  type        = string
}

# ============================================================
# CERTIFICADO PARA APPLICATION GATEWAY (Opcional)
# ============================================================

variable "cert_data" {
  description = "Certificado SSL codificado en base64 (opcional)"
  type        = string
  default     = ""
}

variable "cert_password" {
  description = "Contraseña del certificado SSL (opcional)"
  type        = string
  default     = ""
}

# ============================================================
# BASE DE DATOS SQL SERVER
# ============================================================

variable "sql_server_name" {
  description = "Nombre del servidor SQL"
  type        = string
}

variable "sql_admin_login" {
  description = "Usuario administrador del SQL Server"
  type        = string
}

variable "sql_admin_password" {
  description = "Contraseña del usuario administrador del SQL Server"
  type        = string
  sensitive   = true
}

variable "database_name" {
  description = "Nombre de la base de datos"
  type        = string
}

# ============================================================
# KEY VAULT Y STORAGE
# ============================================================

variable "key_vault_name" {
  description = "Nombre del Key Vault principal"
  type        = string
}

variable "storage_account_name" {
  description = "Nombre de la cuenta de almacenamiento (Blob)"
  type        = string
}

# ============================================================
# ELASTICSEARCH Y OPENTELEMETRY
# ============================================================

variable "enable_elastic" {
  description = "Habilitar Elasticsearch"
  type        = string
  default     = "false"
}

variable "enable_otel" {
  description = "Habilitar OpenTelemetry Collector"
  type        = string
  default     = "true"
}

# ============================================================
# ID de la identidad federada (OIDC) de GitHub Actions
# ============================================================

variable "azure_client_id" {
  description = "Client ID de la identidad federada de GitHub (OIDC)"
  type        = string
}

# ============================================================
# KEY VAULT – ROLES (GitHub OIDC y tu usuario admin)
# ============================================================

variable "github_principal_id" {
  description = "Object ID del Service Principal federado (OIDC) usado por GitHub Actions"
  type        = string
}

variable "admin_user_object_id" {
  description = "Object ID de tu usuario personal en Azure AD (para rol Key Vault Admin)"
  type        = string
}
